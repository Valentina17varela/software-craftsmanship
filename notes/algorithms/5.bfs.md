# Breadth-First Search (BFS)

## 1. Core idea

BFS explores a graph level by level. A FIFO queue ensures that vertices reached with fewer edges are processed first.

```text
        A
      /   \
     B     C
    / \     \
   D   E     F

queue: [A] -> [B,C] -> [C,D,E] -> [D,E,F]
order:  A, B, C, D, E, F
```

```mermaid
flowchart LR
    A((A)) --> B((B))
    A --> C((C))
    B --> D((D))
    B --> E((E))
    C --> F((F))
```

## 2. Invariant and behavior

When a vertex leaves the queue, its shortest distance in number of edges is known for an unweighted graph. Mark vertices visited when enqueuing them, not when removing them.

| Structure | Purpose |
| --- | --- |
| Queue | Preserve level order |
| Visited set | Prevent repeated work and cycles |
| Distance map | Store minimum edge count |
| Parent map | Reconstruct a shortest path |

## 3. Python templates

```python
from collections import deque


def bfs(graph: dict[str, list[str]], start: str) -> list[str]:
  queue = deque([start])
  visited = {start}
  order = []

  while queue:
    node = queue.popleft()
    order.append(node)
    for neighbor in graph[node]:
      if neighbor not in visited:
        visited.add(neighbor)
        queue.append(neighbor)

  return order


graph = {"A": ["B", "C"], "B": ["D", "E"], "C": ["F"], "D": [], "E": [], "F": []}
print(bfs(graph, "A"))  # ['A', 'B', 'C', 'D', 'E', 'F']
```

Shortest distance in an unweighted graph:

```python
from collections import deque


def shortest_distance(graph: dict[int, list[int]], start: int, target: int) -> int:
  queue = deque([(start, 0)])
  visited = {start}

  while queue:
    node, distance = queue.popleft()
    if node == target:
      return distance
    for neighbor in graph[node]:
      if neighbor not in visited:
        visited.add(neighbor)
        queue.append((neighbor, distance + 1))
  return -1


graph = {0: [1, 2], 1: [0, 3], 2: [0, 3], 3: [1, 2, 4], 4: [3]}
print(shortest_distance(graph, 0, 4))  # 3
```

## 4. Complexity and recognition

| Representation | Time | Extra space |
| --- | ---: | ---: |
| Adjacency list | $O(V + E)$ | $O(V)$ |
| Grid with $R \times C$ cells | $O(RC)$ | $O(RC)$ worst case |

Use BFS for minimum moves in an unweighted graph, level-order tree traversal, nearest-target problems, and multi-source spreading simulations.

## 5. Common mistakes

- Using `list.pop(0)`, which is $O(n)$; use `collections.deque`.
- Marking visited too late and enqueuing the same node repeatedly.
- Using BFS for arbitrary weighted shortest paths.
- Forgetting to validate grid boundaries before enqueueing neighbors.
